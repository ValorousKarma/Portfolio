...
const RenderHeader = () => {
	return (
		<h2 className={`${styles.sectionHeadText} py-16 text-center`}>
			My Work.
		</h2>
	);
};
...
const RepoButton = ({ project }) => {
	return (
      ...
      shadow-lg`}
			onClick={() => {
				window.open(project.source_code_link, "_blank", "noreferrer");
			}}
		>
			<LuExternalLink size={"38"} className="mx-auto pt-2" />
		</div>
	);
};
